package com.sx.onereader.utils;

/**
 * Created by Administrator on 2017/3/13.
 */
public class Api {
    public static final String ZHIHU_NEWS = "http://news-at.zhihu.com/";
    public static final String ZHIHU_HISTORY = "http://news.at.zhihu.com/";
    public static final String ZHIHU_DETAIL= "http://daily.zhihu.com/";

    public static final String GUOKR_ARTICLES = "http://apis.guokr.com/handpick/";
    public static final String GUOKR_ARTICLE_LINK = "http://jingxuan.guokr.com/";
    public static final String TODAY_HISTORY_DETAIL = "http://api.juheapi.com/japi/";
    public static final String JOCKER_URL = "http://japi.juhe.cn/joke/content/";
    public static final String TODAY_HISTORY = "http://api.juheapi.com/japi/";
    public static final String SHOWAPI_NEWS= "https://route.showapi.com/";
    public static final String ROBOT= "http://www.tuling123.com/openapi/";
    public static final String Weather ="https://route.showapi.com/";
    public static final String BOOK ="https://api.douban.com/v2/book/isbn/";//查找图书

}
